...
%define BASE    0x1000                                  ; 0x1000:0x0 = 0x1000
%define KSIZE   128                                     ; nombre de secteurs a charger

%define vesa_Global_Info            4096                ; Général VESA capability, la structure sera écrite juste à la fin du code ! placées en 0x0A00:0000 (16b) ou 0x0000:A000
...
%define vesa_Mode_Info (vesa_Global_Info + 256)         ; Ce pointeur pointe "au delà" du programme en 0x0000:8000 + 8192 => 0x0000:A000 -> On écira & lira ici le buffer VESA ici!
...
%define graphic_modes_list (vesa_Mode_Info + 512)       ; zone de 128 0xFFFF pour acceuillir le tampon des modes graphiques disponibles.
...
; Fill 128 16bits fields in graphic_mode_list location with 0xFFFF data
    ...
    mov di, graphic_modes_list                   ; copie de tous les différents modes trouvés dans la variable de 256 octets _graphic_modes_list
...
gdtptr:
    ...
    dd 0  ; base
;--------------------------------------------------------------------
granulosity_coeff: db 0   ; généré après l'obtention des informations sur le mode VESA selectionné
;--------------------------------------------------------------------
bootdrv:  db 0
