...
int			mlx_key_release(int keycode, t_env *e)
{
	...
	if (DEBUG_KEYBOARD)
	{
		printf("keycode %3i [mod]%3i RELEASED -> ", keycode, keycode & 0xFF);
...
int			mlx_key_press(int keycode, t_env *e)
{
	...
	if (DEBUG_KEYBOARD)
	{
		printf("keycode %3i [mod]%3i PRESSED -> ", keycode, keycode & 0xFF);
		print_register(e->keyb);
	}
	return (keycode);
}
...
void		interpolate_switch(t_env *e, unsigned long int m)
{
	...
	{
		asprintf(&s, "Interpolation lineaire: %s",
