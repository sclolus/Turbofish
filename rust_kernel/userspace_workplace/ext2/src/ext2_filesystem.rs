...
impl Ext2Filesystem {
    ...
    pub fn new(f: StdFile) -> Self {
        ...
        let superblock: SuperBlock = disk.disk_read_struct(1024);
        ...
        Self { block_size, superblock, nbr_block_grp, disk }
    }

    /// Try to clone the Ext2Filesystem instance
    pub fn try_clone(&self) -> std::io::Result<Self> {
        Ok(Self { disk: self.disk.try_clone()?, ..*self })
    }
    ...
    pub fn find_block_grp(&mut self, n: u32) -> BlockGroupDescriptor {
        ...
        self.disk.disk_read_struct(block_grp_addr)
    }
    ...
    pub fn find_inode(&mut self, inode: u32) -> (Inode, u32) {
        ...
        (self.disk.disk_read_struct(inode_addr), inode_addr)
    }
    ...
    pub fn find_entry(&mut self, file: &mut File) -> Option<DirectoryEntryHeader> {
        ...
        let dir_header: DirectoryEntryHeader = self.disk.disk_read_struct(base_addr);
        // let ptr = self.disk.disk_read_exact(base_addr + size_of::<DirectoryEntryHeader>() as u32, dir_header.name_length as u32);
        Some(dir_header)
    }

    pub fn iter_entries<'a>(&'a mut self, inode: Inode, inode_addr: u32) -> IoResult<EntryIter<'a>> {
        if unsafe { !inode.type_and_perm.contains(TypeAndPerm::DIRECTORY) } {
            return Err(IoError::NotADirectory);
        }
        Ok(EntryIter { filesystem: self, file: File { curr_offset: 0, inode, inode_addr }, cur_dir_index: 0 })
    }

    fn alloc_block_on_grp(&mut self, n: u32) -> Option<Block> {
        let block_grp_addr: Block = self.block_grp_addr(n);
        ...
        let mut bitmap: [u8; 1024] = self.disk.disk_read_struct(bitmap_addr);
        for i in 0..self.superblock.get_block_per_block_grp() {
            if !bitmap.get_bit(i as usize) {
                bitmap.set_bit(i as usize, true);
                self.disk.write_struct(bitmap_addr + i / 8, &bitmap[(i / 8) as usize]);
                block_grp.allocate_block();
                self.disk.write_struct(self.to_addr(block_grp_addr), &block_grp);
                return Some(block_grp_addr + Block(i + 1));
            }
        }
        None
    }
    ...
    fn inode_data_alloc(&mut self, file: &mut File, alloc: bool) -> Result<u32, IoError> {
        ...
        if block_off >= offset_start && block_off < offset_end {
            ...
            // dbg!(&file.inode);
            return Ok(self.to_addr(none_if_zero(file.inode.direct_block_pointers[block_off as usize])?)
        ...
        if block_off >= offset_start && block_off < offset_end {
            ...
            let singly_indirect = none_if_zero({
            ...
            //     )?
            // };
            let pointer_addr = self.to_addr(singly_indirect) + off * size_of::<Block>() as u32;
            let pointer: Block = none_if_zero({
                let pointer = self.disk.disk_read_struct(pointer_addr);
        ...
        if block_off >= offset_start && block_off < offset_end {
            ...
            let pointer_to_pointer: Block = self.disk.disk_read_struct(
                self.to_addr(file.inode.doubly_indirect_block_pointers) + off * size_of::<Block>() as u32,
            );

            let off = (block_off - offset_start) % blocknumber_per_block as u32;
            let pointer: Block =
                self.disk.disk_read_struct(self.to_addr(pointer_to_pointer) + off * size_of::<Block>() as u32);
        ...
        if block_off >= offset_start && block_off < offset_end {
            ...
            let off = (block_off - offset_start) / (blocknumber_per_block * blocknumber_per_block) as u32;
            let pointer_to_pointer_to_pointer: Block = self.disk.disk_read_struct(
                self.to_addr(file.inode.triply_indirect_block_pointers) + off * size_of::<Block>() as u32,
            );
    ...
    pub fn read(&mut self, file: &mut File, buf: &mut [u8]) -> IoResult<usize> {
        ...
        let data_read = self.disk.disk_read_buffer(data_address, &mut buf[0..offset]);
        ...
        for chunk in buf[offset..].chunks_mut(self.block_size as usize) {
            ...
            let data_read = self.disk.disk_read_buffer(data_address, &mut chunk[0..offset]);
            file.curr_offset += data_read as u32;
