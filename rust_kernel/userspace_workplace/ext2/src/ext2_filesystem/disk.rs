...
impl ReaderDisk {
    ...
    pub fn disk_read_buffer(&mut self, offset: u32, buf: &mut [u8]) -> usize {
        self.0.seek(SeekFrom::Start(offset as u64 + START_OF_PARTITION)).expect("seek failed in disk_read_buffer");
    ...
    pub fn disk_write_buffer(&mut self, offset: u32, buf: &[u8]) -> usize {
        self.0.seek(SeekFrom::Start(offset as u64 + START_OF_PARTITION)).expect("seek failed in disk_write_buffer");
        self.0.write_all(buf).expect("write failed in disk_write_buffer");
