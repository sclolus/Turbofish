...
pub extern "C" fn kmain(multiboot_info: *const MultibootInfo, device_map_ptr: *const DeviceMap) -> u32 {
    ...
    make_somization(1024, kmalloc, kfree, ksize, || 4096).expect("failed sodo 0");
    make_somization(1024, kmalloc, kfree, ksize, || srand::<u32>(16) as usize * 4096).expect("failed sodo 1");
    make_somization(1024, kmalloc, kfree, ksize, || srand::<u32>(32) as usize * 4096).expect("failed sodo 2");
    make_somization(1024, kmalloc, kfree, ksize, || srand::<u32>(64) as usize * 4096).expect("failed sodo 3");
    make_somization(1024 * 4, kmalloc, kfree, ksize, || srand::<u32>(4096) as usize).expect("failed sodo 4");
