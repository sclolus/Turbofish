...
pub mod inode;
use inode::{mode_t, File, Inode, InodeId, InodeNumber, InodeOperations, InodeStatus};
...
impl Dcache {
    pub fn new() -> Self {
        ...
        let root_dentry =
            DirectoryEntry::new("/", Filetype::Directory, InodeId::new(2, 0)).unwrap();
...
impl VirtualFileSystem {
    pub fn new() -> VfsResult<VirtualFileSystem> {
        ...
        new.dcache.add_direntry(DirectoryEntry::new(
            "test",
            Filetype::Directory,
            InodeId::new(3, 0),
        )?);
    ...
    fn pathname_resolution(&mut self, path: &Path) -> Option<&DirectoryEntry> {
        ...
        'walk: loop {
            let component = loop {
                // rewrite all of this with skip_while.
                match components.next() {
                    Some(component) => {
                        if component == "" {
                            continue;
            ...
            let new_direntry = (current_dir_inode.inode_operations.lookup_direntry.unwrap())(
                &current_dir_inode,
                component,
            ); // remove this unwrap
            ...
            if let Some(direntry) = new_direntry {
                ...
                self.dcache
                    .get_direntry_mut(current_dir_id)?
                    .add_direntry(direntry_id)
                    .unwrap(); //remove this unwrap
                let new_inode =
                    (current_dir_inode.inode_operations.lookup_inode.unwrap())(direntry_inode_id)
                        .unwrap();
    ...
    fn add_inode(&mut self, inode: Inode) -> VfsResult<()> {
...
pub fn init() -> VfsResult<VirtualFileSystem> {
    ...
    println!("{}", vfs);
    let result: Result<(), VfsError> = vfs.dcache.walk_tree_mut(&mut |entry| {
        entry.add_direntry(DirectoryEntry::new(
            "lol",
            Filetype::Directory,
            InodeId::new(5, 0),
        )?);;        Ok(())
    ...
    println!(
        "pathname resolution {:?}",
        vfs.pathname_resolution("/test/lol")
    );
    println!(
        "pathname resolution {:?}",
        vfs.pathname_resolution("/test1/lol")
    );
    println!(
        "pathname resolution {:?}",
        vfs.pathname_resolution("/test/dlol")
    );
